...
const ToolWorkspace = () => {
    ...
    const [viewMode, setViewMode] = useState(searchParams.get('mode') || 'do');
    ...
    useEffect(() => {
        if (activeIframe?.contentWindow) {
            activeIframe.contentWindow.postMessage({ type: 'toggle-execution-mode', enabled: isExecutionMode }, '*');
        }
    }, [isExecutionMode, activeIframe]);
    ...
    const [notes, setNotes] = useState<any[]>(() => {
        if (typeof window === 'undefined') return [{ id: 1, content: "Capture your tactical analysis here...", color: "#fef3c7" }];
    ...
    // --- SNIPER / LENS STATE ---
    const [sniperMode, setSniperMode] = useState<'lens' | 'sniper'>('lens');
    ...
    const [drawMode, setDrawMode] = useState<'pen' | 'highlighter' | 'pencil'>('pen');
    ...
    const handleIframeLoad = (e: any) => {
        ...
        try {
            ...
            if (toolId !== 'hoshin') cssRules += `footer, .footer, .bottom-bar, .bottom-nav, #footer, .site-footer, .universal-nav-bar, .workspace-footer, .tool-footer, .authority-bar, .mission-control { display: none !important; }`;
            style.textContent = cssRules;
            doc.head.appendChild(style);

            doc.addEventListener('mousemove', (me: any) => {
                const rect = iframe.getBoundingClientRect();
                handleMouseMoveGlobal({ clientX: me.clientX + rect.left, clientY: me.clientY + rect.top });
                if (activeToolRef.current === 'draw' && isDrawing) {
                    drawAction({ clientX: me.clientX + rect.left, clientY: me.clientY + rect.top });
                }
            });
            doc.addEventListener('mousedown', (me: any) => {
            ...
            doc.addEventListener('wheel', (we: WheelEvent) => {
                if (activeToolRef.current === 'sniper') {
                    we.preventDefault();
        ...
        <div className={`h-screen w-full flex flex-col overflow-hidden relative ${theme === 'light' ? 'bg-slate-50' : 'bg-black'}`}>
            ...
            <div className={`h-16 flex items-center justify-between px-6 z-[900] absolute top-0 left-0 right-0 shell-interactive transition-all duration-300 header-3d`}>
                <div className="flex items-center gap-6 w-1/4">
                    <button onClick={() => router.back()} className={`w-10 h-10 rounded-full border border-nexus-border flex items-center justify-center transition-all group ${theme === 'light' ? 'text-nexus-text-secondary hover:bg-slate-100 hover:text-nexus-text-primary' : 'text-nexus-text-secondary hover:bg-white/5 hover:text-nexus-text-primary'}`}>
            ...
            <div className="flex-1 flex overflow-hidden relative pt-16 p-4 pb-20">
                {/* Attempt Info Overlay (Only if not execution mode, or show subtly in both) */}
                ...
                <div className={`w-full h-full overflow-hidden rounded-2xl border shadow-2xl transition-all duration-300 p-1.5 ${theme === 'light' ? 'bg-white border-slate-200' : 'bg-slate-900 border-white/5'}`}>
                    <iframe
                        src={tool.src}
                        onLoad={handleIframeLoad}
                        title={tool.name}
                        className={`w-full h-full border-none origin-top rounded-xl ring-1 ${theme === 'light' ? 'ring-slate-200' : 'ring-white/10'} ${isExecutionMode ? 'filtering-edu-content' : ''}`}
                        loading="lazy"
                        ...
                        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 30 }} className="fixed top-20 right-8 w-[450px] z-[1300] drop-shadow-2xl shell-interactive">
                            <div className="bg-[#1e293b] rounded-2xl overflow-hidden flex flex-col h-[550px] border border-white/5 shadow-2xl">
                                ...
                                <div className="flex-1 relative flex flex-col" style={{ backgroundColor: notes[activeNoteIndex].color }}>
                                    ...
                                    <div ref={editorRef} contentEditable className="flex-1 w-full p-8 text-black/80 text-sm focus:outline-none font-sans leading-relaxed selection:bg-black/10 overflow-y-auto scrollbar-hide" onBlur={() => { if (editorRef.current) { const updated = [...notes]; updated[activeNoteIndex].content = editorRef.current.innerHTML; saveNotes(updated); } }} dangerouslySetInnerHTML={{ __html: notes[activeNoteIndex].content }} />
                                </div>
                ...
                <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 z-[1000] flex items-center gap-6 p-2 rounded-[2.5rem] bg-black/60 backdrop-blur-2xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] shell-interactive transition-all duration-300`}>
                    ...
                    <button 
                        ...
                        className={`relative flex items-center justify-center gap-3 px-12 h-12 rounded-[2rem] font-black font-orbitron text-[11px] tracking-[0.2em] transition-all duration-500 active:scale-95 group overflow-hidden ${
                            isCompleted 
                    ...
                    >
                        ...
                        {isCompleted ? <CheckCircle2 className="w-4 h-4" /> : <Shield className="w-4 h-4" />}
                        {isCompleted ? 'PROTOCOL SECURED' : 'MARK COMPLETE'}
                    </button>
                    ...
                    <div className="flex items-center gap-2 px-4 border-l border-white/5">
                        ...
                        <button onClick={() => { if (activeIframe?.contentWindow) activeIframe.contentWindow.scrollTo({ top: 0, behavior: 'smooth' }); }} className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center text-slate-400 hover:text-white transition-all"><ArrowUp className="w-4 h-4" /></button>
                    </div>
                </div>
            </div>
            {/* Conditional CSS to hide educational resources in execution mode */}
            {isExecutionMode && (
                <style dangerouslySetInnerHTML={{
                    __html: `
                    iframe.filtering-edu-content { filter: brightness(1.05) contrast(1.1); }
                    /* We inject hide rules into the Iframe if cross-origin allows, 
                       otherwise we rely on the iframe itself reading a 'nexus-execution-mode' message */
                `}} />
            )}
